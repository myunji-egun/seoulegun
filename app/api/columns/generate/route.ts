...
const SYSTEM_PROMPT = `당신은 "서울이건치과"(수원 소재)의 칼럼을 작성하는 의료 카피라이터입니다.
...
5) 이미지 자리: 설명에 도움이 되는 위치 2~4곳에 아래 형태로 자리표시자를 넣습니다(관리자가 나중에 사진을 교체).
   <div class="img-box"><img src="[이미지 설명 URL]" alt="이미지 설명"></div>
  ...
  try {
    ...
    if (!process.env.ANTHROPIC_API_KEY) {
      return NextResponse.json(
        { error: 'AI 변환이 설정되지 않았습니다. 서버에 ANTHROPIC_API_KEY를 추가해주세요.' },
        { status: 503 },
      )
    }
    ...
    if (response.stop_reason === 'refusal') {
      return NextResponse.json({ error: 'AI가 요청을 처리할 수 없습니다. 내용을 확인해주세요.' }, { status: 422 })
    }
